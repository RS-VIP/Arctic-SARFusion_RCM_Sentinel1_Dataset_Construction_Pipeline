...
for zip_file in sorted(os.listdir(shapefile_folder), key=lambda f: int(re.match(r'\d+', f).group())):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # STRICT time-window filter (because client.query may return broader results)
        ...
        client.results["__acq_start_utc"] = pd.to_datetime(client.results["Acquisition Start Date"], utc=True)
        client.results = client.results[
            (client.results["__acq_start_utc"] >= start_dt) &
            (client.results["__acq_start_utc"] <= end_dt)
        ]
        ...
        if not rcm_gdf.empty:
            ...
            if s1_match:
                s1_acq_start = datetime.strptime(
                    s1_match.group(1),
                    "%Y%m%dT%H%M%S"
            ...
            rcm_time = best_match.get("Acquisition Start Date", "")
...
df_best.to_excel("match_rcm_scenes_summary_train.xlsx", index=False)
#--------------------------------------------------------------------------------------------------------------------------------------
