...
"""
Note: after submitting the orders(run the 1st part of the code), wait for the request approval email, and then download the scenes (run the 2nd part of the code)
"""
...
# Part 1 --------------------------------------------------------------------------------------------------------------------------------------
# Search & Retrieve compatible RCM Scenes 
...
def extract_time_range(filename):
    ...
    start = datetime.strptime(match.group(1), "%Y%m%dT%H%M%S").replace(tzinfo=timezone.utc)
    end   = datetime.strptime(match.group(2), "%Y%m%dT%H%M%S").replace(tzinfo=timezone.utc)

    return (
        (start - timedelta(hours=5)).strftime("%Y-%m-%dT%H:%M:%SZ"),
        (end   + timedelta(hours=5)).strftime("%Y-%m-%dT%H:%M:%SZ"),
    )
...
for zip_file in sorted(os.listdir(shapefile_folder), key=lambda f: int(re.match(r'\d+', f).group())):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # STRICT time parsing + filtering
        start_dt = pd.to_datetime(start_date, utc=True)
        end_dt   = pd.to_datetime(end_date, utc=True)

        client.results = client.results.copy()
        client.results["__acq_start_utc"] = pd.to_datetime( client.results["Acquisition Start Date"], errors="coerce", utc=True)
        client.results = client.results.dropna(subset=["__acq_start_utc"])
        client.results = client.results[
            (client.results["__acq_start_utc"] >= start_dt) & (client.results["__acq_start_utc"] <= end_dt)
        ]

        print(f"Scenes after strict time filter: {len(client.results)}")
 
        # filter by polarization (Only HH, HV)
        filtered = client.results[
            (client.results["Polarization in Product"].str.contains("HH", na=False) & client.results["Polarization in Product"].str.contains("HV", na=False)) &
        ...
        print(f"Scenes after metadata filter: {len(filtered)}")
        ...
        if not filtered.empty:
            ...
            # Compute intersection area directly
            ...
            if not rcm_gdf.empty:
                s1_match = re.search(r'_(\d{8}T\d{6})_(\d{8}T\d{6})_', zip_file)
                s1_acq_start_dt = pd.to_datetime(s1_match.group(1), format="%Y%m%dT%H%M%S", utc=True)

                rcm_gdf["time_diff_hours"] = ((rcm_gdf["__acq_start_utc"] - s1_acq_start_dt).abs().dt.total_seconds() / 3600)
        else:
        ...
        if not rcm_gdf.empty:
            ...
            print(best_match[["Granule", "__acq_start_utc", "time_diff_hours", "overlap_percent"]])
            ...
            if s1_match:
                s1_acq_start = datetime.strptime( s1_match.group(1), "%Y%m%dT%H%M%S").strftime("%Y-%m-%d %H:%M:%S")
            else:
                s1_acq_start = ""

            rcm_time = best_match["__acq_start_utc"]

            if pd.notnull(rcm_time):
                rcm_time = pd.to_datetime(rcm_time).tz_localize(None)


            row_data = {
                "Sentinel-1 shapefile": zip_file,
                "S1 time": s1_acq_start,
                "RCM time": rcm_time,
