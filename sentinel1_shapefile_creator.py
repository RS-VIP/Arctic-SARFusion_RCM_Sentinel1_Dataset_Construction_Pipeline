# 0- Create Sentinel-1s' shapefiles to search RCM products based on them
...
folder_path = "/home/n2azad/projects/rrg-dclausi/ai4arctic/dataset/ai4arctic_raw_test_v3"
output_folder = "test_dataset_shapefiles"
...
# checking shapefiles
DIR = "./test_dataset_shapefiles/"  
report = []                          
for fn in sorted(os.listdir(DIR)):
    path = os.path.join(DIR, fn)
    row  = {"file": fn, "is_zip": False, "has_shp": False, "error": ""}

    try:
        row["is_zip"] = zipfile.is_zipfile(path)
        if row["is_zip"]:
            with zipfile.ZipFile(path) as z:
                row["has_shp"] = any(f.endswith(".shp") for f in z.namelist())
        else:
            row["error"] = "not a zip file"
    except Exception as e:       
        row["error"] = str(e)
    report.append(row)
df = pd.DataFrame(report)
bad_zip = df[~df["is_zip"]]
missing   = df[df.is_zip & ~df.has_shp]
ok        = df[df.is_zip & df.has_shp]

print(f"TOTAL files scanned : {len(df)}")
print(f"Valid ZIP + .shp : {len(ok)}")
print(f"Not ZIP or corrupt: {len(bad_zip)}")
print(f"ZIP w/o .shp    : {len(missing)}")
